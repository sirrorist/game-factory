...
| `tools/gf.test.ts` | офлайн-проверка, неизменяемость версий, экспорт, ошибки CLI, `gf new`, `--prebuilt`, `check-prod` против поддельного прода | 9 |
| `tools/docs.test.ts` | ссылки `D-`/`П-` ведут на существующие записи, номера не повторяются | 1 |
...
| `apps/play-server/test/server.test.ts` | заголовки, ETag и 304 (игры, обложки, архивы), методы, обход пути, симлинки, MIME, хосты, SW, архивы, обложки и их кеш по версии в адресе | 16 |
...
Итого 49 юнит + 20 e2e (проверено: `pnpm check`, 2026-09-26, сессия №5).
