...
| Веб-адрес только 80/443 (`ufw deny in to 2.26.198.231`) | проверка с *** (***) | *** не достаёт до VPS-1 **ни на одном** адресе (и `***:443` закрыт) - проверка непоказательна; правило владелец откатил на всякий случай и вернул; проверено с телефона через мобильную сеть: хаб открывается, игра работает (2026-09-26) |
